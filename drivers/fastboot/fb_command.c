...
static void set_gateway(char *, char *);
...
static const struct
{
	...
	[FASTBOOT_COMMAND_OEM_SET_GATEWAY] = {
		.command = "oem set_gateway",
		.dispatch = set_gateway,
		.privileged = 1
	},
...
static void set_gateway(char *cmd_parameter, char *response)
{
	if (!cmd_parameter)
	{
		fastboot_fail("Expected command parameter", response);
	...
	ethernet_settings.gateway = string_to_ip(cmd_parameter);
	ethernet_settings.crc = compute_ethernet_settings_crc(&ethernet_settings);
	...
	fastboot_response("OKAY", response, "Gateway IP set to %s", buf);
}
