...
void write_default_ethernet_settings(struct ethernet_settings *ethernet_settings)
{
	...
	ethernet_settings->gateway = string_to_ip(CONFIG_DEFAULT_GATEWAY);
...
void apply_ethernet_settings(const struct ethernet_settings *ethernet_settings)
{
	...
	net_gateway = ethernet_settings->gateway;
	...
	ip_to_string(ethernet_settings->gateway, ip_string);
	env_set("gatewayip", ip_string);
}
...
int check_ethernet_settings(const struct ethernet_settings *ethernet_settings, const char **feedback)
{
	...
	else if (ethernet_settings->gateway.s_addr == 0 ||
			 ethernet_settings->gateway.s_addr == MAX_UINT32)
	{
		feedback_str = get_dp_err_feedback(DEVPARAMS_ERR_ETH_SETTINGS_INVALID_GATEWAY);
		ret = 1;
	}
