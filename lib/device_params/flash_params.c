...
// Compile-time check that flash_params are not located in first 5 MiB of flash
CASSERT(CONFIG_FLASH_DEVICE_PARAMS_ADDR >= MIN_OFFSET_FROM_START);

// Forward-declarations of static functions
static struct spi_nor *probe_flash(void);
static int write_params_to_flash(struct spi_nor *flash, const struct flash_params *flash_params);
...
static struct spi_nor *probe_flash(void)
{
	...
	// Device parameters must fit in flash
	if (flash->size < CONFIG_FLASH_DEVICE_PARAMS_ADDR + sizeof(struct flash_params))
	{
...
static int write_params_to_flash(struct spi_nor *flash, const struct flash_params *flash_params)
{
	...
	ret = spi_flash_erase(flash, CONFIG_FLASH_DEVICE_PARAMS_ADDR,
						  erase_blocks * flash->erase_size);
	if (ret != 0)
		return ret;

	ret = spi_flash_write(flash, CONFIG_FLASH_DEVICE_PARAMS_ADDR,
						  sizeof(*flash_params), flash_params);
...
static int init_device_flash_params(void)
{
	...
	if (spi_flash_read(flash, CONFIG_FLASH_DEVICE_PARAMS_ADDR,
					   sizeof(device_flash_params), &device_flash_params) != 0)
	{
		printf("Failed to read flash params (%u bytes) from flash addr: %u\n",
			   sizeof(device_flash_params), CONFIG_FLASH_DEVICE_PARAMS_ADDR);
		return 1;
	}
...
int write_modified_flash_params(void)
{
	...
	if (write_params_to_flash(flash, &modified_device_flash_params) != 0)
	{
		printf("Writeback of flash_params failed\n");
		return 1;
	}
