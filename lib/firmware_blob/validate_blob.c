...
static const char *const validate_blob_err_feedback[VALIDATE_BLOB_ERR_COUNT] = {
	...
	[VALIDATE_BLOB_ERR_INTEGRITY_VERIFICATION_FAILED] = "verification of blob signature failed",
	[VALIDATE_BLOB_ERR_INTEGRITY_UNALIGNED_BLOB] = "blob is not aligned to 4-byte boundary",
	[VALIDATE_BLOB_ERR_INTEGRITY_INVALID_MAGIC] = "invalid magic at start of blob",
	[VALIDATE_BLOB_ERR_INTEGRITY_INVALID_BLOB_LEN] = "invalid blob size, blob appears to be truncated",
	[VALIDATE_BLOB_ERR_INTEGRITY_TOO_MANY_SIG_BYTES] = "unsupported blob signature size",
	...
	[VALIDATE_BLOB_ERR_UNAUTHORIZED_SW_TYPE] = "unauthorized SW type for board boot policy",
	[VALIDATE_BLOB_ERR_BOARD_PARTITION_MISMATCH] = "incorrect SW type for board partition",
	[VALIDATE_BLOB_ERR_SIGNATURE_MISSING_WHEN_REQUIRED] = "SW signature is missing and required by board boot policy",
	[VALIDATE_BLOB_ERR_INTERNAL] = "internal error occured in blob validation library",
};
...
static const struct authorization_record boot_policy_sw_type_authorization_matrix[BOARD_BOOT_POLICY_COUNT][SW_TYPE_COUNT] = {
...
validate_blob_err_t validate_blob(const struct validate_blob_input *validate_blob_input, const char **feedback)
{
	...
	ret = verify_blob_integrity(validate_blob_input->blob, validate_blob_input->blob_len, &blob_header, &has_valid_signature);
	if (ret != 0)
		goto out;
	...
	const struct authorization_record *authorization_record =
		&boot_policy_sw_type_authorization_matrix[validate_blob_input->board_boot_policy][blob_header.sw_type];
	...
	if (authorization_record->board_partition != validate_blob_input->board_partition)
	{
		ret = VALIDATE_BLOB_ERR_BOARD_PARTITION_MISMATCH;
		goto out;
	}

	if (has_valid_signature != 1 && authorization_record->allow_skip_signature_check != 1)
	{
